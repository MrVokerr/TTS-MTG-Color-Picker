-- VOKER → Purple (companion to Seat Color Picker)
-- Duplicate of UGD-PURPLE.lua for SteamID STEAM_ID.
-- Left-click:
...
local function applyMatImage(player)
  if type(MAT_IMAGE_URL) ~= 'string' or MAT_IMAGE_URL == '' then
    player.broadcast('Set MAT_IMAGE_URL in VOKER-PURPLE.lua to your hosted mat PNG.', {1, 0.5, 0.3})
    return false
  end
...
local function showButton()
  self.clearButtons()
  self.createButton({
    click_function = 'clickVokerPurple',
    function_owner = self,
    label = 'Purple',
    tooltip = 'VOKER PURPLE NOW.\nRight-click to restore original seat.',
    position = { 0, 0.15, 0 },
    rotation = { 0, 180, 0 },
    width = 600,
    height = 600,
...
function clickVokerPurple(obj, color, alt)
